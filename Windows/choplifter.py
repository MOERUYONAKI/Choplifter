...
def choplifter(size : tuple = (1280, 720)):
    ...
    cms_font = py.font.Font('Windows\\assets\\Fonts\\amd64_microsoft-windows-f..ruetype-comicsansms_31bf3856ad364e35_10.0.22621.1_none_3deaef772e20c404\\comic.ttf', 24)
    ...
    music = Music("Windows\\assets\\Songs\\Audiomachine - By the Hand of the Mortal.mp3", 0.25)
    music.play()

    # Menu assets
    menu = add_asset('Windows\\assets\menu_bg.jpg')

    choplifter_txt = add_asset('Windows\\assets\\choplifter_txt.png') 
    choplifter_txt[1].center = (int(round(size[0] / 2, 0)), int(round(0.06 * size[1], 0)))

    play_txt = add_asset('Windows\\assets\\play_txt.png') 
    play_txt[1].center = (int(round(size[0] / 2, 0)), int(round(size[1] / 2.15, 0)))

    quit_txt = add_asset('Windows\\assets\\quit_txt.png') 
